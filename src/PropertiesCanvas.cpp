...
void CPropertiesCanvas::AddProperty(Property* p, wxPGProperty* parent_prop)
{
	...
	case ChoicePropertyType:
		{
			...
			//wxPGProperty *new_prop = new wxEnumProperty(p->GetShortString(), wxPG_LABEL, array_string, ((PropertyChoice*)p)->m_initial_index);
			wxPGProperty *new_prop = new wxEnumProperty(p->GetShortString(), wxPG_LABEL, array_string);
			new_prop->SetValue( ((PropertyChoice*)p)->m_initial_index ); //Fix assert "uiIndex < m_nCount" failed
			if(!p->property_editable())new_prop->ChangeFlag(wxPG_PROP_READONLY, true);
			Append( parent_prop, new_prop, p );
		}
		break;
	case VertexPropertyType:
		{
			//wxPGProperty* new_prop = new wxParentProperty(p->GetShortString(),wxPG_LABEL);
			//http://wxpropgrid.sourceforge.net/docs/pg14/index.htm
			//As a new feature in version 1.3.1, basicly any property can have children. There are few limitations, however
			wxPGProperty* new_prop = new wxStringProperty(p->GetShortString(),wxPG_LABEL);
			Append( parent_prop, new_prop, p );
			double x[3];
